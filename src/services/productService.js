...
const persistRemoteProducts = (products) =>
  persistRemoteProductsCache(products.map((p) => normalizeRemoteProductInput(p)));
...
const resolveCategoryId = async (product) => {
  ...
  const match = categories.find(
    (cat) =>
      String(cat.name ?? '').trim().toLowerCase() === normalizedCategoryName ||
      String(cat.slug ?? '').trim().toLowerCase() === normalizedCategoryName
  );
...
const buildRemoteAdminPayload = async (product) => {
  ...
  if (!categoryId) throw new Error('La categoría seleccionada no existe en el backend.');
  return {
...
function getProductById(productId) {
  ...
  return loadProducts().find((p) => p.id === normalizedId) ?? null;
}
  ...
  return persistProducts(
    currentProducts.map((p) =>
      p.id === updatedProduct.id
        ? normalizeProductInput({ ...p, ...updatedProduct }, currentProducts)
        : p
    )
  );
}
...
function deleteProduct(productId, currentProducts = loadProducts()) {
  return persistProducts(currentProducts.filter((p) => p.id !== normalizeId(productId)));
}

function getProductsAsync(filters = {}) {
  if (!appConfig.useRemoteApi) return toAsyncResult(() => getProducts(filters));

  return requestJson(`/products${buildQueryString(filters)}`, { method: 'GET' }).then((response) =>
    applyFilters(persistRemoteProducts(extractCollection(response)), filters)
  );
}

function getProductByIdAsync(productId) {
  const normalizedId = normalizeId(productId);
  if (!appConfig.useRemoteApi) return toAsyncResult(() => getProductById(normalizedId));
...
function createProductAsync(product, currentProducts = loadProducts()) {
  if (!appConfig.useRemoteApi) return toAsyncResult(() => createProduct(product, currentProducts));

  return buildRemoteAdminPayload(product).then((body) =>
    requestJson('/admin/products', { method: 'POST', body, token: loadSessionToken() }).then(() =>
      getProductsAsync()
    )
  );
}
...
function updateProductAsync(updatedProduct, currentProducts = loadProducts()) {
  if (!appConfig.useRemoteApi) return toAsyncResult(() => updateProduct(updatedProduct, currentProducts));
...
function deleteProductAsync(productId, currentProducts = loadProducts()) {
  const normalizedId = normalizeId(productId);
  if (!appConfig.useRemoteApi) return toAsyncResult(() => deleteProduct(normalizedId, currentProducts));
