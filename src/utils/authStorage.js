...
const normalizeRole = (role) => {
  ...
  if (normalizedRole === ADMIN_ROLE || normalizedRole === 'ADMIN') return ADMIN_ROLE;
  return DEFAULT_ROLE;
};
...
const sanitizeSessionUser = (user) => {
  ...
  if (!normalizedUser.id || !normalizedUser.email) return null;
...
const ensureAdminUser = (users) => {
  const normalizedUsers = Array.isArray(users)
    ? users.map(normalizeUser).filter((u) => u.id && u.fullName && u.email && u.password)
    : [];
  const hasAdmin = normalizedUsers.some((u) => u.role === ADMIN_ROLE);
...
export function findUserByEmail(email) {
  ...
  return loadUsers().find((u) => u.email === normalizedEmail) ?? null;
}
...
export function updateUser(userId, updates) {
  ...
  saveUsers(nextUsers);
  if (updatedSessionUser) saveSessionUser(updatedSessionUser);
